...
public class TestMcpService : IDisposable
{
    ...
    private async Task SimulateTestExecution(TestSession session, CancellationToken cancellationToken)
    {
        ...
        for (int i = 0; i < suites.Length && !cancellationToken.IsCancellationRequested; i++)
        {
            ...
            {
                Name = suite.Name,
                TestsSucceeded = !suite.ShouldFail,
                ExecutionTime = TimeSpan.FromMilliseconds(suite.Duration),
                PassedTests = suite.ShouldFail ? 5 : 10,
                FailedTests = suite.ShouldFail ? 3 : 0
            };
            ...
            var status = result.TestsSucceeded ? "✅ **Passed**" : "❌ **Failed**";
            await StreamNotification($"{status}", 
                $"`{suite.Name}` - {result.PassedTests} passed, {result.FailedTests} failed");
        }
    ...
    private static string CreateCompletionSummary(TestSession session)
    {
        ...
        var totalTests = session.Results.Sum(r => r.PassedTests + r.FailedTests);
        var passedTests = session.Results.Sum(r => r.PassedTests);
        var failedTests = session.Results.Sum(r => r.FailedTests);
